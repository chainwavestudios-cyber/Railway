...
app.get("/health", (req, res) => {
  res.status(200).send("Orion Engine Live");
});
...
wss.on("connection", (clientWs, req) => {
  ...
  const deepgramApiKey =
    parameters.k || process.env.DEEPGRAM_API_KEY;
  ...
  /* -------------------------------------------------------
     HANDLE INCOMING EVENTS FROM TWILIO / STREAM
  ------------------------------------------------------- */
    ...
    try {
      ...
      if (msg.event === "start") {
        ...
        dgWs.on("open", () => {
          ...
          /* -------------------------------------------------------
             DEEPGRAM SETTINGS + STRICT SCRIPT CONTROL
          ------------------------------------------------------- */
          dgWs.send(
            JSON.stringify({
              type: "Settings",
...
`,
                  functions: [
                    {
                      ...
                      description:
                        "Lead agreed to a scheduled call with Chris",
                      parameters: {
                        type: "object",
                        properties: {
                          day: { type: "string" },
        ...
        dgWs.on("message", async (data, isBinary) => {
          ...
          if (dgMsg.type === "FunctionCallRequest") {
            ...
            // Add system tag
            details.tag = "lead_qualified";

            try {
              await fetch(
                "https://agentbman2.base44.app/api/functions/postCallSync",
                {
                  method: "POST",
                  headers: {
...
server.listen(PORT, "0.0.0.0", () => {
  console.log(`🚀 Orion Engine Running on Port ${PORT}`);
});
